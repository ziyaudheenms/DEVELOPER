    ...
    <AnimatePresence mode="wait">
      ...
      >
        {navItems.map((navItem: any, idx: number) => (
          ...
          >
            <span className="block sm:hidden"><Image src={navItem.icon} alt="logo" height={20} width={20}/></span>
            <span className="hidden sm:block text-sm">{navItem.name}</span>
          </Link>
        ))}
        <Link href='#home'>
        <button className="border text-sm font-medium relative  border-red-600 bg-black-100 px-4 py-2 rounded-full">
